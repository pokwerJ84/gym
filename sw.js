/* Gym Tracker service worker v128 */
const VERSION="v128";
...
const CORE_ASSETS=[
  ...
  "./hotfix-v126.css","./hotfix-v126.js","./hotfix-v128.css","./hotfix-v128.js",
...
self.addEventListener("install",event=>event.waitUntil((async()=>{
  ...
  const critical=["./index.html","./hotfix-v126.css","./hotfix-v126.js","./hotfix-v128.css","./hotfix-v128.js"];
...
async function injectV128(response){
  ...
  html=html.replace(/const VERSION=125;/g,"const VERSION=128;");
  html=html.replace(/class="app-version">v125</g,'class="app-version">v128<');
  if(!html.includes("hotfix-v126.css"))html=html.replace("</head>",'<link rel="stylesheet" href="./hotfix-v126.css?v=128">\n</head>');
  if(!html.includes("hotfix-v128.css"))html=html.replace("</head>",'<link rel="stylesheet" href="./hotfix-v128.css?v=128">\n</head>');
  if(!html.includes("hotfix-v126.js"))html=html.replace("</body>",'<script src="./hotfix-v126.js?v=128"></script>\n</body>');
  if(!html.includes("hotfix-v128.js"))html=html.replace("</body>",'<script src="./hotfix-v128.js?v=128"></script>\n</body>');
...
async function navigationNetworkFirst(request){
  const cache=await caches.open(CORE_CACHE);
  try{const response=await fetch(request,{cache:"no-store"});if(response.ok)await cache.put("./index.html",response.clone());return injectV128(response)}
  catch(_){const cached=(await cache.match("./index.html"))||(await cache.match("./"));return injectV128(cached)}
}
...
self.addEventListener("notificationclick",event=>{event.notification.close();event.waitUntil((async()=>{const list=await clients.matchAll({type:"window",includeUncontrolled:true});if(list.length){await list[0].focus();return}await clients.openWindow("./?v=128&finishWorkout=1")})())});
