/* Gym Tracker service worker v130 */
const VERSION="v130";
...
const CORE_ASSETS=[
  ...
  "./hotfix-v126.css","./hotfix-v129.css","./hotfix-v130.css","./v129-core.js","./v129-detail.js","./v129-cloud.js","./v130-core.js",
...
self.addEventListener("install",event=>event.waitUntil((async()=>{
  ...
  const critical=["./index.html","./hotfix-v126.css","./hotfix-v129.css","./hotfix-v130.css","./v129-core.js","./v129-detail.js","./v129-cloud.js","./v130-core.js"];
...
async function injectV130(response){
  ...
  html=html.replace(/const VERSION=125;/g,"const VERSION=130;");
  html=html.replace(/class="app-version">v125</g,'class="app-version">v130<');
  if(!html.includes("hotfix-v126.css"))html=html.replace("</head>",'<link rel="stylesheet" href="./hotfix-v126.css?v=130">\n</head>');
  if(!html.includes("hotfix-v129.css"))html=html.replace("</head>",'<link rel="stylesheet" href="./hotfix-v129.css?v=130">\n</head>');
  if(!html.includes("hotfix-v130.css"))html=html.replace("</head>",'<link rel="stylesheet" href="./hotfix-v130.css?v=130">\n</head>');
  if(!html.includes("v129-core.js"))html=html.replace("</body>",'<script src="./v129-core.js?v=130"></script>\n<script src="./v129-detail.js?v=130"></script>\n<script src="./v129-cloud.js?v=130"></script>\n<script src="./v130-core.js?v=130"></script>\n</body>');
  else if(!html.includes("v130-core.js"))html=html.replace("</body>",'<script src="./v130-core.js?v=130"></script>\n</body>');
...
async function navigationNetworkFirst(request){
  const cache=await caches.open(CORE_CACHE);
  try{const response=await fetch(request,{cache:"no-store"});if(response.ok)await cache.put("./index.html",response.clone());return injectV130(response)}
  catch(_){const cached=(await cache.match("./index.html"))||(await cache.match("./"));return injectV130(cached)}
}
...
self.addEventListener("notificationclick",event=>{event.notification.close();event.waitUntil((async()=>{const list=await clients.matchAll({type:"window",includeUncontrolled:true});if(list.length){await list[0].focus();return}await clients.openWindow("./?v=130&finishWorkout=1")})())});
