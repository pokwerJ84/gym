...
function dashboard(){
 ...
 let sessionRows=ssn.length?ssn.slice(0,8).map(s=>{const total=s.done+s.skipped,rate=total?Math.round(s.done/total*100):null;return '<div class="v170-session-row"><div class="v170-session-copy"><strong>'+esc(s.name.replace(/^[^A-Za-zÀ-ž0-9]+\\s*/,''))+'</strong><small>'+s.count+'× · '+(s.count?Math.round(s.minutes/s.count):0)+' min '+(cs()?'průměr':'avg')+'</small><div class="v170-session-bar"><i style="width:'+(rate==null?0:rate)+'%"></i></div></div><div class="v170-session-rate"><b>'+(rate==null?'–':rate+'%')+'</b><span>complete</span></div></div>'}).join(''):'<div class="empty">'+(cs()?'Zatím bez tréninků.':'No workouts yet.')+'</div>';
